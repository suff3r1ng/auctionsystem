...
class action
{
	...
	function signup()
	{
		...
		if($_FILES['profile']['name']){
			$move = move_uploaded_file($_FILES['profile']['tmp_name'], "../../uploads/".$_FILES['profile']['name']);
			$img="uploads/".$_FILES['profile']['name'];
			$data .= ", img = '$img' ";
		}
		...
		if($save = $this->db->query("INSERT INTO users set $data")){
			return "1";
		}
	...
	function update_user()
	{
		...
		if($save = $this->db->query("UPDATE users set $data where id = '{$_SESSION['login_id']}' ")){
			return 1;
		}
		return 2;
	...
	function update_password(){
		...
		if (empty("$password")) {
			...
			$chk = $this->db->query("SELECT * FROM users WHERE id = '$user' and password = '$password'")->num_rows;
			if ($chk > 0) {
				return 4;
			}else{
				return 5;	
	...
	function save_settings()
	{
		...
		$data .= ", email = '$email' ";
		$data .= ", contact = '$contact' ";
		$data .= ", about_content = '" . htmlentities(str_replace("'", "&#x2019;", $about_content)) . "' ";
		if ($_FILES['img']['tmp_name'] != '') {
			$fname = strtotime(date('y-m-d H:i')) . '_' . $_FILES['img']['name'];
